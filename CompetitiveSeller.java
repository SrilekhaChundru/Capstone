...
/*
    ...
    public List<AmazonData> getCompetitiveSellersDetails(String asinNo)
    {
        ...
        Query competitorQuery = entityManager.createQuery("SELECT  amazonData FROM AmazonData amazonData where amazonData.queryId =: queryNumber and amazonData.uniqueCrawledId is not null  order by amazonData.createdDate,amazonData.rank asc ");
...

    {
        ...
        try{
            if(number==1)
            {
                ...
                competitorQuery = entityManager.createQuery("SELECT  amazonData FROM AmazonData amazonData where amazonData.queryId =: queryNumber and amazonData.uniqueCrawledId is not null order by amazonData.createdDate,amazonData.rank  ");
            ...
            else if (number==2)
            {
                ...
                competitorQuery = entityManager.createQuery("SELECT  amazonData FROM AmazonData amazonData where amazonData.queryId =: queryNumber and amazonData.uniqueCrawledId is not null order by amazonData.createdDate,amazonData.rank DESC");
                competitorQuery.setParameter("queryNumber",queryNumber);
                competitorQuery.setMaxResults(maxResult);
                competitorDetails = competitorQuery.getResultList();
...
return competitorDetails;
    ...
    public List<AlgorithmData> getCompetitorKeywords(String crawledId, int flag, String type)
    {
        List<AlgorithmData> competitorKeywords = null;

        try
        {

            EntityManager entityManager = entityManagerFactory.createEntityManager();
            Query algorithmKeywords = entityManager.createQuery("SELECT  keywordData FROM AlgorithmData keywordData  WHERE keywordData.summaryFlag = :flag AND keywordData.uniqueCrawledId = :crawlerId AND keywordData.type= :type order by keywordData.score desc");

            algorithmKeywords.setParameter("flag", flag);
            algorithmKeywords.setParameter("crawlerId", crawledId);
            algorithmKeywords.setParameter("type", type);

            competitorKeywords = algorithmKeywords.getResultList();
            entityManager.close();
